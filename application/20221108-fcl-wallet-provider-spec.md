...
# FCL Wallet Provider Specification
...
A FLIP is a document (or set of documents) that defines or describes a Flow Improvement Proposal. A FLIP is a design document providing information to the Flow community, or describing a new feature for Flow or its processes or environment. The FLIP should provide a concise technical specification of the feature and a rationale for the feature.
...
This FLIP proposes a standard protocol and schema definition that applications, wallets, users,can use to interact with the Flow blockchain. This FLIP defines a standard, language-agnostic interface to...
An FCL compatible wallet uses and conforms to the FCL Wallet Provider Specification.
...
Currently there is only a draft proposal and several implementations of an FCL Wallet Provider Spec. At best, most implementations
employ a simple

## <a id="userbenefit"></a>User Benefit

How will users (or other contributors) benefit from this work? What would be the headline in the release notes or blog post?

Creation of an FCL Wallet Provider Spec can...

- enables easy...
  This spec will lead to more
- more wallets and service providers...
- applications that integrate...

This will make FCL more intrinsically useful and valuable to the end users of Flow.

It will also open up the possibility to standardize on some shared...

Indirectly, users will benefit from the fact that...
...
The aim of this flip is to:

- Synthesize the work presented in earlier FCL Wallet Provider drafts into a standard spec for use in any language and regardless of implementation
- Provide an opportunity for the community, developers, and stakeholders to weigh in, contribute to the protocol before the v1 spec
- A spec that defines and encourages Interoperability and open standards

This proposal will make it possible to...

Want to add your wallet to FCL Discovery and gain access to the entire Flow Ecosystem?

If you can

- Authn
- confirm a user's identity and grant access to their wallet keys or safely store their keys locally
- return a list of FCL Services (type, endpoint, method)
- Handle service requests for the following types
- authn,
- authz,
- user-signature,
  -Authz - sign a transaction, submitted via FCL, on behalf of the user - return a signed transaction
  ...
  Wallets will be much more likely to...
  Without it...
...
### <a id="serviceobject"></a>Service Object
...
- If design changes existing API or creates new ones, the design owner should create
  ...
  Some things to consider related to the tutorial: - It should show the usage of the new feature in an end to end example
  (i.e. from the browser to the execution node).
  Many new features have unexpected effects in parts far away from the place of
  change that can be found by running through an end-to-end example. - This should be written as if it is documentation of the new feature,
  i.e., consumable by a user, not a Flow contributor. - The code does not need to work (since the feature is not implemented yet)
